...
@section('content')
<div class="max-w-3xl" x-data="invoiceForm(@js($invoice->items->map(fn($i) => ['product_name' => $i->product_name, 'description' => $i->description, 'quantity' => $i->quantity, 'unit_price' => (float)$i->unit_price])), {{ (float)$invoice->discount }}, @js($invoice->extras->map(fn($e) => ['label' => $e->label, 'amount' => (float)$e->amount])))">
    <form method="POST" action="{{ route('admin.invoices.update', $invoice) }}">
            ...
            <div class="card-body p-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div class="form-control sm:col-span-2">
                        ...
                        <textarea name="customer_address" class="textarea textarea-bordered" rows="2">{{ old('customer_address', $invoice->customer_address) }}</textarea>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="card-body p-4">
                <div class="grid sm:grid-cols-2 gap-4">
                    <div class="form-control">
                        ...
                        <input type="number" name="discount" x-model.number="discount" @input="calcTotal()" class="input input-bordered" min="0">
                    </div>
                    <div class="form-control">
                        <label class="label"><span class="label-text">Catatan</span></label>
                        <textarea name="notes" class="textarea textarea-bordered" rows="2">{{ old('notes', $invoice->notes) }}</textarea>
                    </div>
                </div>
                <div class="divider"></div>
                <div class="flex flex-col items-end gap-1 text-sm">
                    <div class="flex gap-4 justify-between w-56">
                        <span>Subtotal:</span><span x-text="'Rp ' + formatNum(subtotal)"></span>
                    </div>
                    ...
                    <div class="flex gap-4 justify-between w-56 font-bold text-base border-t pt-1 mt-1">
                        <span>Total:</span><span x-text="'Rp ' + formatNum(total)"></span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="flex gap-3 justify-end">
            <a href="{{ route('admin.invoices.show', $invoice) }}" class="btn btn-ghost">Batal</a>
...
@push('scripts')
<script>
function invoiceForm(initialItems, initialDiscount, initialExtras) {
    return {
        items: initialItems,
        ...
        total: 0,
        init() { this.calcTotal(); },
        addItem() { this.items.push({ product_name: '', description: '', quantity: 1, unit_price: 0 }); },
        removeItem(index) { this.items.splice(index, 1); this.calcTotal(); },
        addExtra() { this.extras.push({ label: '', amount: 0 }); },
        removeExtra(index) { this.extras.splice(index, 1); this.calcTotal(); },
        calcTotal() {
